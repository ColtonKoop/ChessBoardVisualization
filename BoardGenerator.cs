...
public class BoardGenerator : MonoBehaviour
{
        ...
        if (timeLeft < 0)
        {
            if (IsValidLabel())
            {
                if (screenshotsTaken < generatorCount)
                {
                    ...
                    Vector3 coordsA1 = generatorCamera.WorldToViewportPoint(positionForSquare(Square.A1)); //Could be causing my rotation issue
    ...
    //I think this is where the actual piece generation happens
