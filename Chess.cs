...
namespace Chess
{
    ...
    public class Board
    {
        ...
        public static Board Random(RandomStrategy strategy)
        {
            ...
            //generate JSON file
            string filePath = "/Users/Colton/Desktop/Chess Screenshots/" + (Storage.boardNumber-1) + ".json";
            System.Text.StringBuilder builder = new System.Text.StringBuilder("{\"config\": {");
            foreach (Square sq in System.Enum.GetValues(typeof(Square)))
            {
                ...
                {
                    builder.Append("\"" + sq + "\": ");
                    builder.Append("\"" + pieceMap[sq].pieceType.ToString().ToLower() + "_" + pieceMap[sq].color.ToString().ToLower()[0] + "\", ");
                }
            }
            builder.Length -= 2;
            builder.Append("}}");
